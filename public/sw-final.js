// Service Worker Final para Catledan SaaS
...
self.addEventListener('fetch', (event) => {
  ...
  console.log('📦 Service Worker: Interceptando request:', request.url);
  
  // Interceptar navegación
  if (request.mode === 'navigate') {
    console.log('🌐 Service Worker: Interceptando navegación a', request.url);
    
    event.respondWith(
      fetch(request)
        .then(response => {
          console.log('✅ Service Worker: Respuesta de red exitosa');
          return response;
        })
        .catch(error => {
          console.log('❌ Service Worker: Error de red, mostrando página offline');
  ...
  event.respondWith(
    fetch(request)
      .then(response => {
        console.log('✅ Service Worker: Recurso cargado desde red');
        return response;
      })
      .catch(error => {
        console.log('❌ Service Worker: Error cargando recurso desde red');
        return new Response('Recurso no disponible offline', { status: 404 });
      })
  );
});

console.log('✅ Service Worker: Listo para interceptar navegación');
